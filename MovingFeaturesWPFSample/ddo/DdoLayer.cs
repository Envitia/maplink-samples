...
namespace MovingFeaturesWPFSample.ddo
{
    ...
    /// </summary>
    public class DdoLayer
    {
        public Envitia.MapLink.DDO.TSLNObjectDataLayer DdoDataLayer { get; set; } = new Envitia.MapLink.DDO.TSLNObjectDataLayer();
        ...
        public void Start(Envitia.MapLink.TSLN2DDrawingSurface surface, Action? requestRender = null)
        {
            ...
            // Add the DDO layer to the drawing surface.
            // The DDO layer is a special type of data layer that is used to manage Dynamic Data Objects (DDOs) in the MapLink SDK.
            if (surface.addDataLayer(DdoDataLayer, "DDOLayer") == false)
            {
            ...
            // Make the DDO layer visible and detectable.
            ...
            // Create a number of Dynamic Data Objects moving between randomly assigned departure and destination airports.
            ...
            for (uint i = 0; i < numDdos; ++i)
            {
                // Create a Dynamic Data Object with a random departure and destination airport.
                var dynamicObject = new DynamicObject();
                DdoDataLayer.addDDO(dynamicObject);
        ...
        {
            var ddos = DdoDataLayer.getDDOList();
            foreach (var ddo in ddos)
            {
                DynamicObject dynamicObject = (DynamicObject)ddo;
                if (ddo == null) continue;

                double latToGo = (dynamicObject.To?.Item1 ?? 0) - (dynamicObject.Location?.Item1 ?? 0);
                double lonToGo = (dynamicObject.To?.Item2 ?? 0) - (dynamicObject.Location?.Item2 ?? 0);

                bool notAtDestination = latToGo != 0 || lonToGo != 0;
                if (notAtDestination)
                {
                    // Move the DDO along the same path towards it's destination
                    Envitia.MapLink.TSLNCoordinateConverter.greatCircleDistancePoint(dynamicObject.Location?.Item1 ?? 0, dynamicObject.Location?.Item2 ?? 0, dynamicObject.Heading, dynamicObject.Velocity, out double endLat, out double endLon);
                    int x = 0;
                    int y = 0;
                    DrawingSurface?.latLongToTMC(endLat, endLon, out x, out y);
                    dynamicObject.move(new Envitia.MapLink.TSLNCoord(x, y), true);
                    dynamicObject.Location = new Tuple<double, double>(endLat, endLon);
                }
            }

            DdoDataLayer.notifyChanged(true);
