...
namespace MovingFeaturesWPFSample.Tracks
{
    ...
    public class TracksLayer
    {
        ...
        public void Start(Envitia.MapLink.TSLN2DDrawingSurface? surface, Action? requestRender = null)
        {
            ...
            // Add the Track Display Manager to the drawing surface.
            // The Track Display Manager is a special type of data layer that is used to manage tracks in the MapLink SDK.
            if (TrackDisplayManager.addDrawingSurface(surface, Identifier()) == -1)
            {
                ...
                {
                    throw new Exception($"Failed to add drawing surface to track manager.\n{errorCode}: {errorMessage}");
                }
                else
                {
                    throw new Exception("Failed to add drawing surface to track manager");
            ...
            // Make the Tracks layer visible and detectable.
        ...
        private void DispatcherTimer_Tick(object sender, EventArgs e)
        {
            ...
            {
                trackIds[i] = track.Id;
                lats[i] = track.To?.Item1 ?? 0;
                lons[i] = track.To?.Item2 ?? 0;

                double latToGo = (track.To?.Item1 ?? 0) - (track.Location?.Item1 ?? 0);
                double lonToGo = (track.To?.Item2 ?? 0) - (track.Location?.Item2 ?? 0);

                bool notAtDestination = latToGo != 0 || lonToGo != 0;
                if (notAtDestination)
                {
                    // Move the track along the same path towards it's destination
                    Envitia.MapLink.TSLNCoordinateConverter.greatCircleDistancePoint(track.Location?.Item1 ?? 0, track.Location?.Item2 ?? 0, track.GetTrack().heading(), track.Velocity, out double endLat, out double endLon);
                    lats[i] = endLat;
                    lons[i] = endLon;
