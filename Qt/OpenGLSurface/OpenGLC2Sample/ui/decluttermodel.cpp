  ...
  switch( role )
  {
    ...
    case Qt::CheckStateRole:
    {
      ...
      if( !node->m_featureName.isEmpty() )
      {
        ...
        m_surface->getDeclutterStatus( node->m_featureName.toUtf8(), &declutterStatus, node->m_layerName.toUtf8() );
    ...
    case Qt::CheckStateRole:
      {
        ...
        if( m_surface->setDeclutterStatus( node->m_featureName.toUtf8(), value == Qt::Checked ? TSLDeclutterStatusOn : TSLDeclutterStatusOff,
                                           node->m_layerName.toUtf8() ) )
        {
...
void DeclutterModel::addLayerFeatures( const QString &layerName, TSLDataLayer *layer )
{
  ...
  DeclutterNode *layerNode = NULL;
  for( size_t i = 0; i < m_rootNode->m_children.size(); ++i )
  {
    if( m_rootNode->m_children[i]->m_layerName == layerName )
    {
      layerNode = m_rootNode->m_children[i];
      break;
    }
  }
  ...
  if( !layerNode )
  {
    // No current node exists for this layer, create a new one
    layerNode = new DeclutterNode( m_rootNode, (int)m_rootNode->m_children.size(), layerName, QString(), layerName );
