...
TrackUpdater::TrackUpdater( TrackManager *manager )
  ...
  , m_coordSys( NULL )
{
  // Create the TSLAPP6AHelper
  const char *maplHome = TSLUtilityFunctions::getMapLinkHome();
  if( !maplHome )
  {
    QMessageBox::critical( NULL, "Cannot load APP6A Symbol configuration file",
      "This sample requires the MAPL_HOME environmental variable to be set", QMessageBox::Ok );
  }
  else
  {
    QString configLocation( QString::fromUtf8( maplHome ) );
    configLocation += "/config/app6aConfig.csv";
    loadSymbolConfig( configLocation );
  }
...
{
  size_t numExistingTracks = m_tracks.size();
  if ( numExistingTracks > 0 )
  {
    // Delete all current Tracks
    for( size_t i = 0; i < numExistingTracks; ++i )
    {
      delete m_tracks[i];
      m_tracks[i] = 0;
    }
...
void TrackUpdater::selectTrack(double lat, double lon, double tmcPerDU)
{
  size_t numTracks = m_tracks.size();
  for (size_t i = 0; i < numTracks; ++i)
  {
    ...
    if (currentTrack == NULL)
      continue;
    if (currentTrack->intersects(lat, lon, tmcPerDU, m_coordSys))
    {
