...
bool Track::intersects(double lat, double lon, double tmcPerDU, const TSLCoordinateSystem *coordSys) const
{
  if (!coordSys)
    return false;
  ...
  TSLTMC x;
  TSLTMC y;
  coordSys->latLongToTMC(lat, lon, &x, &y);
  ...
  double symbolHeight = m_type.height();
  double expansionFactor = symbolHeight / 2.0;
  expansionFactor = expansionFactor * tmcPerDU;
  displayExtent.expand(expansionFactor);
  bool intersected = displayExtent.contains(TSLCoord(x, y));
  return intersected;
}
