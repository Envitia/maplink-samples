...
//! Map to load.
//! ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
void MapLinkWidget::loadLayer(const char *filename)
{
	if (m_application)
	{
		//! Load the map/file
		std::string err_msg;
		if (!m_application->loadLayer(filename, err_msg))
		{
			QMessageBox::critical(parentWidget(), "Cannot load layer", QString::fromUtf8(err_msg.c_str()));
			return;
		}
...
void MapLinkWidget::activate_Trackball_Mode()
{
	m_application->activate_Trackball_Mode();
}
void MapLinkWidget::activate_Select_Mode()
{
	m_application->activate_Select_Mode();
}
void MapLinkWidget::activate_CreatePolygon_Mode()
{
	m_application->activate_CreatePolygon_Mode();
}
void MapLinkWidget::activate_CreatePolyline_Mode()
{
	m_application->activate_CreatePolyline_Mode();
}
void MapLinkWidget::activate_CreateText_Mode()
{
	m_application->activate_CreateText_Mode();
}
void MapLinkWidget::activate_CreateSymbol_Mode()
{
	m_application->activate_CreateSymbol_Mode();
}
void MapLinkWidget::activate_CreateExtrudedPolygon_Mode()
{
	m_application->activate_CreateExtrudedPolygon_Mode();
}
void MapLinkWidget::activate_CreateExtrudedPolyline_Mode()
{
	m_application->activate_CreateExtrudedPolyline_Mode();
}
void MapLinkWidget::activate_DeleteGeometry_Mode()
{
	m_application->activate_DeleteGeometry_Mode();
}

void MapLinkWidget::interaction(int i, bool reactivate)
{
	m_application->interaction(i, reactivate);
}
int MapLinkWidget::currentInteractionMode()
{
	return m_application->currentInteractionMode();
}

void MapLinkWidget::startTracks(int numTracks)
{
	m_application->startTracks(numTracks);
}
void MapLinkWidget::removeTracks()
{
	m_application->removeTracks();
}
...
void MapLinkWidget::ViewChangedCallBack(viewChangedCallBack func)
{
	m_application->ViewChangedCallBack(func);
}
void MapLinkWidget::SelectedTrackCallBack(selectedTrackCallBack func)
{
	m_application->SelectedTrackCallBack(func);
}

//! The MapLink drawing surface
envitia::maplink::earth::Surface3D* MapLinkWidget::drawingSurface() 
{ 
	return m_application->drawingSurface();
}
// object to manage the tracks
TracksManager& MapLinkWidget::tracksManager()
{
	return m_application->tracksManager();
}
