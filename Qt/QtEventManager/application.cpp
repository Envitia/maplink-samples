  ...
  m_clientManager(NULL)
{
  ...
  //! these files.
  //! ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  if (!TSLDrawingSurface::loadStandardConfig())
  {
    QMessageBox::information(m_parentWidget, "Initialisation Error",
      "Failed to load standard config", QMessageBox::Cancel);
  }

  if (!TSLDrawingSurface::setupSymbols("tslsymbolsAPP6A.dat"))
  {
    QMessageBox::information(m_parentWidget, "Initialisation Error",
      "Failed to setup APP6A symbols", QMessageBox::Cancel);
  }
...
void Application::create()
{
  ...
  if (m_modeManager == NULL)
  {
    ...
    m_modeManager->addMode(new InteractionModeTracks(this, ID_TOOLS_TRACKS), false);
  ...
  {
    m_clientManager = new ClientManager(m_parentWidget, m_drawingSurface);
    m_clientManager->createTrackManager();
  }
...
void Application::activateTracksMode()
{
  //! Activate the zoom interaction mode
  if (m_modeManager)
  {
    m_modeManager->setCurrentMode(ID_TOOLS_TRACKS);
  }
}
...
void Application::viewChanged(TSLDrawingSurface* drawingSurface)
{
  if (m_clientManager)
  {
    m_clientManager->updateViewExtent(drawingSurface);
  }
}
...
//! set the call back to update the GUI for reseting interaction modes.
ClientManager* Application::getClientManager()
{
  return m_clientManager;
}

//! handles clicking left click in the tracks mode.
bool Application::onTracksModeLeftClick(TSLDeviceUnits x, TSLDeviceUnits y)
{
  if (m_clientManager)
  {
    return m_clientManager->handleTracksModeLeftClick(x, y);
  }
  return false;
}