...
Application::Application(QGLWidget *widget) 
: m_mapDataLayer(NULL),
  m_stdDataLayer(NULL),
...
bool Application::loadMap(const char *mapFilename)
{
  ...
  if (mapFilename && mapFilename[0] != '\0' )
  {
    ...
    {
      qWarning("Could not load map data %s", mapFilename);
      return false;
    }
    ...
    TSLSimpleString msg;
    TSLThreadedErrorStack::errorString( msg, "Loading map\n" ) ;
    if ( !msg.empty() )
    {
      qWarning(msg.c_str());
      TSLThreadedErrorStack::clear();
...
#define MT_RENDERER
    ...
    // Quality Settings
    m_configuration->textureFilterNearest(false);// GL_NEAREST (default, occasionally faster on old hardware,
                                       // but you may lose fine detail) or GL_LINEAR (sometimes slower, but
  ...
  // Display any errors that have occurred
  TSLSimpleString msg;
  TSLThreadedErrorStack::errorString( msg, "Cannot initialise view\n" ) ;
  if (!msg.empty())
  {
    qWarning(msg.c_str());
    TSLThreadedErrorStack::clear();
...
bool Application::OnMouseMove(unsigned int, bool, bool, int mx, int my)
{
      ...
      && ( m_grabbed || abs( mx - m_rmb.m_x ) > 3 || abs( my - m_rmb.m_y ) > 3 ) )
  {
    ...
    TSLDeviceUnits du_offset_x = m_lastGrabPoint.m_x - mx ;
    TSLDeviceUnits du_offset_y = my - m_lastGrabPoint.m_y ;
...
bool Application::createPolygon( TSLTMC x, TSLTMC y, TSLDrawingSurface * ds )
{
  ...
  int yellow = TSLDrawingSurface::getIDOfNearestColour( 255, 255, 0 ) ;
  int black  = TSLDrawingSurface::getIDOfNearestColour( 0, 0, 0 ) ;
...
bool Application::createSymbol( TSLTMC x, TSLTMC y, TSLDrawingSurface * )
{
  ...
  int green = TSLDrawingSurface::getIDOfNearestColour( 0, 255, 0 ) ;
